...
protected:
  ...
  void SetUp() override {
    // HostMemory* src_host_memory = new HostMemory(device_id_src, mem_type_);
    // // 源HostMemory对象 HostMemory* des_host_memory = new
    // HostMemory(device_id_des, mem_type_);  // 目标HostMemory对象
  }
...
TEST_F(HostMemoryTest, CopyHostToBuffer_HappyPath) {
  ...
  EXPECT_EQ(
      src_host_memory->copy_host_to_device(destination, source, sizeof(source)),
      status_t::SUCCESS);
...
TEST_F(HostMemoryTest, CopyHostToBuffer_Nullptr) {
  ...
  char destination[20] = {0};
  EXPECT_EQ(
      src_host_memory->copy_host_to_device(nullptr, source, sizeof(source)),
      status_t::ERROR);
  EXPECT_EQ(src_host_memory->copy_host_to_device(destination, nullptr,
                                                 sizeof(source)),
            status_t::ERROR);
}
...
TEST_F(HostMemoryTest, CopyBufferToHost_HappyPath) {
  ...
  char destination[20] = {0};
  EXPECT_EQ(
      src_host_memory->copy_device_to_host(destination, source, sizeof(source)),
      status_t::SUCCESS);
  EXPECT_STREQ(destination, "Hello, World!");
}
...
TEST_F(HostMemoryTest, CopyBufferToHost_Nullptr) {
  ...
  char destination[20] = {0};
  EXPECT_EQ(
      src_host_memory->copy_device_to_host(nullptr, source, sizeof(source)),
      status_t::ERROR);
  EXPECT_EQ(src_host_memory->copy_device_to_host(destination, nullptr,
                                                 sizeof(source)),
            status_t::ERROR);
}
...
TEST_F(HostMemoryTest, CopyBufferToBuffer_HappyPath) {
  char source[] = "Hello, World!";
  char destination[20] = {0};
  EXPECT_EQ(src_host_memory->copy_device_to_device(destination, source,
                                                   sizeof(source)),
            status_t::SUCCESS);
  EXPECT_STREQ(destination, "Hello, World!");
}
...
TEST_F(HostMemoryTest, CopyBufferToBuffer_Nullptr) {
  ...
  char destination[20] = {0};
  EXPECT_EQ(
      src_host_memory->copy_device_to_device(nullptr, source, sizeof(source)),
      status_t::ERROR);
  EXPECT_EQ(src_host_memory->copy_device_to_device(destination, nullptr,
                                                   sizeof(source)),
            status_t::ERROR);
}
