...
status_t HddtMemory::init(){
    return this->memoryClass->init();
}

status_t HddtMemory::free(){
    return this->memoryClass->free();
}
...
std::unique_ptr<Memory> HddtMemory::createMemoryClass(memory_type_t mem_type){
    switch(mem_type) {
        case memory_type_t::CPU:
            return std::make_unique<HostMemory>(this->hddtDeviceId, this->hddtMemoryType);
...
// get memory type 
memory_type_t HddtMemory::get_MemoryType(){
    return this->hddtMemoryType;
}

// get init status
status_t HddtMemory::get_init_Status(){
    return this->initStatus;
}

// get device id
int HddtMemory::get_DeviceId(){
    return this->hddtDeviceId;
}

// reset device id and memory type
status_t HddtMemory::set_DeviceId_and_MemoryType(int device_id, memory_type_t mem_type){
