...
status_t RDMACommunicator::Write(void *addr, size_t length) {
  ...
  sret = process_work_completion_events(this->client_completion_channel, &wc, 1,
                                       &wc_count);
  if (sret!=status_t::SUCCESS || wc_count != 1) {
...
status_t RDMACommunicator::setup_server() {
  ...
  // 4. prepare server's buffer mr
  ibv_access_flags access = static_cast<const ibv_access_flags>(IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ |
...
status_t RDMACommunicator::start_server() {
  ...
  if (sret == status_t::ERROR) {
    logError("Failed to get cm event, ret = %d.", sret);
    return sret;
  }
  ...
  this->server_qp_init_attr->qp_type = IBV_QPT_RC;
  ...
  ret = rdma_create_qp(this->server_cm_id, this->server_newconnection_pd,
                       this->server_qp_init_attr);
  if (ret) {
    logError("Failed to create QP due to errno.");
    return status_t::ERROR;
  }
...
status_t RDMACommunicator::setup_client() {
  ...
  logInfo("Trying to connect to server at : %s port: %d \n",
          inet_ntoa(this->client_addr.sin_addr), ntohs(this->client_addr.sin_port));
  ...
  if (!this->client_cq) {
    logError("Failed to create CQ, errno: %d.");
    return status_t::ERROR;
  }
  logDebug("CQ created at %p with %d elements.", client_cq, client_cq->cqe);
  ret = ibv_req_notify_cq(client_cq, 0);
  if (ret) {
    logError("Failed to request notifications, errno: %d.");
    return status_t::ERROR;
  }
  ...
  if (this->client_newserver_metadata_attr.address == 0) {  // ? todo
    logError("Failed to setup the server metadata mr.");
    return status_t::ERROR;
  }
  ...
  sret = this->post_work_request(
      ...
      ibv_wr_opcode::IBV_WR_RDMA_READ, ibv_send_flags::IBV_SEND_INLINE, NULL,
      NULL, false);
  if (sret != status_t::SUCCESS) {
    logError("Failed to pre-post the receive buffer.");
    return status_t::ERROR;
  }
  logDebug("Receive newserver metadata is successful");
...
status_t RDMACommunicator::start_client() {
  ...
  sret = this->post_work_request(this->client_qp,
                                 ...
                                 IBV_SEND_SIGNALED, NULL, NULL, true);
  if (sret != status_t::SUCCESS) {
  ...
  int wc_count;
  sret = this->process_work_completion_events(this->client_completion_channel, wc, 2,
                                              &wc_count);
  if (sret != status_t::ERROR || wc_count != 2) {
 ...
 * : is_send: ibv_post_send or ibv_post_recv
 */
status_t RDMACommunicator::post_work_request(
  ...
  if (is_send) {
    ...
    ret = ibv_post_send(this->server_newconnection_qp, &newconnection_wr,
                        &bad_newconnection_wr);
    ...
    ret = ibv_post_recv(this->server_newconnection_qp, &newconnection_wr,
                        &bad_newconnection_wr);
  }
...
status_t RDMACommunicator::process_work_completion_events(
  ...
  *wc_count = 0;
  do {
    ret = ibv_poll_cq(cq_ptr /* the CQ, we got notification for */,
                      max_wc - *wc_count /* number of remaining WC elements*/,
                      wc + *wc_count /* where to store */);
    if (ret < 0) {
      logError("Failed to poll cq for wc.");
      return status_t::ERROR;
    }
    *wc_count += ret;
  } while (*wc_count < max_wc);
  logDebug("%d WC are completed.", *wc_count);
...
status_t RDMACommunicator::server_accept_newconnection() {
  ...
  sret =
      post_work_request(this->server_newconnection_qp,
                        ...
                        ibv_send_flags::IBV_SEND_INLINE, NULL, NULL, false);
...
status_t RDMACommunicator::server_send_metadata_to_newconnection() {
  ...
  sret = process_work_completion_events(this->server_completion_channel, &wc, 1,
                                        &wc_count);
  if (sret != status_t::SUCCESS) {
  ...
  sret = post_work_request(
      ...
      IBV_WR_SEND, IBV_SEND_SIGNALED, NULL, NULL, true);
  if (sret != status_t::SUCCESS) {
